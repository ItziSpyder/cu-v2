...
import io.github.itzispyder.pdk.utils.misc.Hotbar;
...
public class CrystalAura extends Module {
    ...
    private boolean checkEntities(Player player, Hotbar hotbar, Location bLoc) {
        ...
        for (Entity entity : bLoc.getWorld().getNearbyEntities(bLoc,2,2,2)) {
            if (entity != player && entity instanceof LivingEntity && !entity.isDead() && hotbar.contains(Material.END_CRYSTAL)) {
                ...
                if (block.getType().equals(Material.AIR) && hotbar.contains(Material.OBSIDIAN)) {
                    hotbar.deduct(Material.OBSIDIAN, false);
    ...
    public static void crystalAt(Player player, Hotbar hotbar, Location loc) {
        ...
        hotbar.deduct(Material.END_CRYSTAL, false);
